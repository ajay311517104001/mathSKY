...
const QAadd= () => {
  ...
  const onSave = ()=>{
   // if(ques &&  options && ans && location.state.chapterName && location.state.id){
        console.log("the subject is ", ques,options,ans,location.state.chapterName, location.state.id)
      ...
      .then((res)=>{
       ...
       history("/QAset/QAkeyin", {state:{_id:location.state.id , flag:"addnew", chapterName:location.state.chapterName}});
       // setData(res)
      })
   // }
  ...
  const onUpdate = ()=>{
    ...
    let data ={
      ...
      if( location.state.item._id && ques.length!==0 && options && location.state.data.quesId && uans>=0 && location.state.chapter){
        let data ={
            id:location.state.item._id,
            ques:  ques,
            options:options,
          ...
          .then((res)=>{
            ...
            if(res.message=="success"){
                 history("/QAset/QAkeyin", {state:{_id:location.state.item._id , flag:"addnew", chapterName:location.state.chapter}});
...
if(location.state.item){
            ...
            <div className="bottom">
              ...
              <div className="right"  >
                ...
                <div className="formInput"  style={{width:'100%'}} >
                      ...
                      <input  value={ques} style={{width:'100%', height:'40%', marginTop:'2%'}} type="textarea"   onChange={(e)=>{
                   ...
                   {/* <h1>{standard}</h1> */}
                   <div style={{backgroundColor:"#6439ff", padding:7, color:'white',width:'100%'}}>
           ...
           <div className="formInput"  style={{width:'100%', display:'flex', flexDirection:'row', justifyContent:'space-evenly'}} >
                   {  options.map((item,index)=>{
                       return( <div style={{display:'flex',flexDirection:'column'}}>
                           ...
                           <input type="radio" value={item} checked={options[uans]==item} name="ans" onChange={()=>{
                               setUans(index)
                              // console.log("the balue is ",index)
                               

                               }} style={{height:'20px', width:'20px'}} /> 
                              <div style={{marginLeft:'5%'}}> option {index+1}  </div>
                       
                               </div>
                    
                    
                        <input value={item} style={{width:'45%', height:'40%', marginTop:'2%'}} type="textarea"   onChange={(e)=>{
    ...
    return (
        <div className="new">
         ...
         <Button onClick={ ()=> history("/QAset/QAkeyin", {state:{_id:location.state.id , flag:"addnew", chapterName:location.state.chapterName}})}>back</Button>
         </div>
            </div>
            <div className="bottom">
              {/* <div className="left">
              ...
              <div className="right"  >
                ...
                <div className="formInput"  style={{width:'100%'}} >
                      <label> Question </label>
                      <input style={{width:'100%', height:'40%', marginTop:'2%'}} type="textarea"   onChange={(e)=>{
                                setQues(e.target.value)
                                // (e.target.value) 
                   ...
                   {  options.map((item,index)=>{
                       return( <div style={{display:'flex',flexDirection:'column'}}>
                           <div style={{display:'flex', flexDirection:'row'}}>
                           <input type="radio" value="Male" name="ans" onChange={()=>{
                               setAns(index)
                        ...
                        <input  style={{width:'45%', height:'40%', marginTop:'2%'}} type="textarea"   onChange={(e)=>{
                        //    setOptions(e.target.value) 
                ...
                <br/>
                <br/>
              <center> <button style={{   paddingInline:30 , paddingTop:10,paddingBottom:10}} onClick={onSave}>save</button>
