...
const SignUpForm = () => {
  ...
  useEffect(()=>{
	gapi.load('auth2', ()=> {
     gapi.auth2.init({clientId:"291717498611-daceb28n13btmnk0d6q7onv1mqalplpd.apps.googleusercontent.com"})
  },[])
})
...
const onSuccess = (res)=>{
console.log("this is on success",res)
const data ={
	token:res.tokenId
}
 googleAuthApi(data)
 .then((res)=>{
	 console.log("the response of the google auth is ", res)
	 localStorage.setItem("accessToken", JSON.stringify(res.accessToken));
	 localStorage.setItem("username", JSON.stringify(res.username));
	 localStorage.setItem("userId", JSON.stringify(res.userId));
