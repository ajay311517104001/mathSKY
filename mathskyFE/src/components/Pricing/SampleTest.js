...
const SampleTest = ({testmodule,questionset,totalQuestions,UserId,ProductId,moduleId,ResultScreenProps}) => {
  ...
  const fun = (mode) => {
   // console.log("the option is",question.ans,option)
    ...
    setOption(-1);
  //  console.log("the count is ",score);
    if (count <= Number(totalQuestions)) {
      setCount(count + 1);
      setQuestion(questions[count]);
     // console.log("the timer is", timer);
      ...
      if (timer == 60 && !mode) {
        let totalTimee = totalTime + Number(60);
       console.log("the audomatic time add 60 is", totalTimee);
        setTotalTime(totalTimee);
      }
  ...
  const onSubmit =(mode)=>{
       console.log("subit function is called")
      setLoading(true)
      if (questions[count - 1].ans == option) {
        setScore(score + 1);
      }else{
     ...
     console.log("the before submit timer is ", totalTime);
     let tt
     if(mode){
       tt = totalTime + 60 ;
     }else{
       tt = totalTime + (60 - Number(timer));
     } 
    
      console.log("the after submit timer is ", tt);
     // setTotalTime(tt);
      setFullscreen(false)
          const data ={
        ProductId:ProductId,
  ...
  useEffect(() => {
    if(count < Number(totalQuestions)){
     //.log("the useeffect count is ",count)
      const interval = setInterval(fun, 60000);
      return () => clearInterval(interval);
    }
    // else{
    //   // let totalTimee = totalTime + Number(60);
    //   //   console.log("the im the culprit is", totalTimee);
    //   //   setTotalTime(totalTimee);
    // }
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (count < Number(totalQuestions)) {
       ...
       // console.log("the timer is  ", timer-1 )
        if (timer > 0) {
  ...
  if(loading){
    return(
      <div style={{height:'100vh', backgroundColor:'#02203c', display:'flex', justifyContent:'center', alignItems:'center'}}>
